...
Application.$controller("GooglemapsController", ["$scope", "Utils", "$element", "NgMap", "$timeout",
    function ($scope, Utils, $element, NgMap, $timeout) {
        ...
        $scope.$on('mapInitialized', function(event, evtMap) {
            ...
            refresh();
            var mapInfo = $scope.maps[0].directionsRenderers[0].directions.routes[0].legs[0];
            $scope.origin = mapInfo.start_address;
            $scope.destination = mapInfo.end_address;
            $scope.distance = mapInfo.distance.text;
            $scope.duration = mapInfo.duration.text;
        });
